...
function createKnight(name) {
  ...
  const stage = {
      ...
      doAttack(attacking,attacked) {
          //vamos la criar a instrução
          if(attacking.life <= 0 || attacked.life <= 0) {
            log.addMessage('Alguém ta morto, não pode atacar');
            return;
          }
          //criando fator de ataque e defesa
          const attackFactor = (Math.random()* 2).toFixed(2);
          const defenseFactor = (Math.random()* 2).toFixed(2);

          //agora criamos a influencia deles na força de fato do atacante e defesa do defensor

          const actualAttack = attacking.attack * attackFactor;
          const actualDefense = attacked.defense * defenseFactor;
          //agora fazemos a verificação
          if(actualAttack >  actualDefense) {
            attacked.life -= actualAttack;
            //fazemos aqui uma verificação extra, como nao fizemos a verifcação da vida dele la em cima, 
  ...
  //abaixo do stage vamos criar um objeto de log, nele tem a lista dos logs, a função para add a mensagem e a função para exibir essas mensagens.
  ...
  //função que criei para atualizar a pagina
